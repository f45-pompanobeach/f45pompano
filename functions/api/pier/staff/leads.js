...
export async function onRequestGet({request,env}){
  ...
  try{
    ...
    const q=await db.prepare(`SELECT id,created_at,updated_at,first_name,last_name,email,phone,zip,is_local,marketing_opt_in,event_sms_consent,confirmation_code,confirmed_at,sms_delivery_status,sms_error_code,prize,prize_saved_at,prize_text_status,prize_text_message_id,prize_text_error_code FROM event_leads WHERE event_key=? ORDER BY created_at DESC LIMIT 500`).bind(EVENT_KEY).all();
