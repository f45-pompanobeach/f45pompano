...
export async function onRequestPost({request,env}){
  ...
  const lead=await db.prepare('SELECT id FROM event_leads WHERE id=? AND event_key=? LIMIT 1').bind(id,event.event_key).first();
  if(!lead)return json({ok:false,error:'not_found'},404);
  await db.prepare('UPDATE event_leads SET notes=?,updated_at=? WHERE id=? AND event_key=?').bind(notes||null,now,id,event.event_key).run();
