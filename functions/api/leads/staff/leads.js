...
export async function onRequestGet({request,env}){
  ...
  const q=await db.prepare(`SELECT id,created_at,updated_at,first_name,last_name,email,phone,zip,is_local,marketing_opt_in,event_sms_consent,confirmed_at,prize,prize_text_status,notes,COALESCE(followup_status,'new') AS followup_status,followup_updated_at FROM event_leads WHERE event_key=? ORDER BY created_at DESC LIMIT 1000`).bind(event.event_key).all();
