import {QR_KITS,adminAuthorized,cleanKit,createUniqueSlug,ensureSchema,eventDb,getAdminPinHash,json,sha256Hex} from '../_shared.js';
...
async function codeAvailable(env,code,excludeKey=null){
  const db=eventDb(env),hash=await sha256Hex(code);
  const adminHash=await getAdminPinHash(env);
  if(hash===adminHash)return false;
  const row=await db.prepare(`SELECT event_key FROM lead_events WHERE staff_code_hash=? AND archived=0 ${excludeKey?'AND event_key<>?':''} LIMIT 1`)
    .bind(...(excludeKey?[hash,excludeKey]:[hash])).first();
  return !row;
}
...
export async function onRequestGet({request,env}){
  ...
  const q=await db.prepare(`SELECT e.event_key,e.name,e.slug,e.event_date,e.start_time,e.end_time,e.qr_kit,e.enabled,e.archived,e.created_at,e.updated_at,
...
export async function onRequestPost({request,env}){
  ...
  if(action==='create'){
    ...
    await db.prepare(`INSERT INTO lead_events (event_key,name,slug,event_date,start_time,end_time,staff_code_hash,qr_kit,enabled,archived,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,1,0,?,?)`)
      .bind(eventKey,name,slug,date,start,end,hash,kit,now,now).run();
    return json({ok:true,event:{event_key:eventKey,name,slug,event_date:date,start_time:start,end_time:end,qr_kit:kit,staff_code:code}});
  }
  const eventKey=String(body.event_key||'').trim();
  if(!eventKey)return json({ok:false,error:'missing_event'},400);
  const existing=await db.prepare('SELECT * FROM lead_events WHERE event_key=? LIMIT 1').bind(eventKey).first();
  if(!existing)return json({ok:false,error:'not_found'},404);
  if(existing.event_key==='2026-09-12-pier-cleanup'&&action!=='unarchive'){
    if(action==='archive')return json({ok:true,unchanged:true});
  }
  ...
  if(action==='update'){
    ...
    await db.prepare('UPDATE lead_events SET name=?,event_date=?,start_time=?,end_time=?,qr_kit=?,updated_at=? WHERE event_key=?').bind(name,date,start,end,kit,now,eventKey).run();
    return json({ok:true});
  }
  if(action==='reset_code'){
    const code=String(body.staff_code||'');
    if(!/^\d{4}$/.test(code))return json({ok:false,error:'invalid_code',message:'Enter a 4-digit event code.'},400);
    ...
    await db.prepare('UPDATE lead_events SET staff_code_hash=?,updated_at=? WHERE event_key=?').bind(await sha256Hex(code),now,eventKey).run();
    return json({ok:true,staff_code:code});
  }
  if(action==='archive'){
    await db.prepare('UPDATE lead_events SET archived=1,enabled=0,qr_kit=NULL,updated_at=? WHERE event_key=?').bind(now,eventKey).run();
    return json({ok:true});
  }
  if(action==='unarchive'){
    await db.prepare('UPDATE lead_events SET archived=0,enabled=1,updated_at=? WHERE event_key=?').bind(now,eventKey).run();
    return json({ok:true});
  }
