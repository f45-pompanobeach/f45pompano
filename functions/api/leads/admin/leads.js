...
export async function onRequestGet({request,env}){
  ...
  let event={event_key:'general',name:'General Leads',slug:null,event_date:null,start_time:null,end_time:null,qr_kit:null,archived:0};
  if(eventKey&&eventKey!=='general'){
    event=await db.prepare('SELECT event_key,name,slug,event_date,start_time,end_time,qr_kit,archived FROM lead_events WHERE event_key=? LIMIT 1').bind(eventKey).first();
    if(!event)return json({ok:false,error:'event_not_found'},404);
  }
  const key=eventKey||'general';
  const q=await db.prepare(`SELECT id,created_at,updated_at,first_name,last_name,email,phone,zip,is_local,marketing_opt_in,event_sms_consent,confirmed_at,prize,notes FROM event_leads WHERE event_key=? ORDER BY created_at DESC LIMIT 1000`).bind(key).all();
